...
public class EncryptionService : IEncryptionService
{
    ...
    public byte[] DecryptByteArray(string data, EncryptionValue eV, string passPhrase)
    {
        byte[] bytes = Encoding.ASCII.GetBytes(eV.InitVector);
        byte[] rgbSalt = Encoding.ASCII.GetBytes(eV.SaltValue);
        byte[] buffer = Convert.FromBase64String(data);
    ...
    /// <summary>
    /// Encrypts byte array value "data" with the EncryptionValues generated by the API with "SHA256" Rfc2898DeriveBytes using AES with a passPhrase from external source.
    /// </summary>
    /// <param name="data"></param>
    /// <param name="eV"></param>
    /// <param name="passPhrase"></param>
    /// <returns>Encrypted String Value</returns>
    public string EncryptByteArray(byte[] data, EncryptionValue eV, string passPhrase)
    {
        byte[] bytes = Encoding.ASCII.GetBytes(eV.InitVector);
        byte[] rgbSalt = Encoding.ASCII.GetBytes(eV.SaltValue);
